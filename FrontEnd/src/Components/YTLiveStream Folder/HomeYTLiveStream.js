...
import YTLiveStreamDetails from './YTLiveStreamDetails';
import CreateYTLiveStreamPost from './CreateYTLiveStreamPost';

const HomeYTLiveStream = () => {
  ...
  const handleDelete = async (postId) => {
    ...
    if (isConfirmed) {
      try {
        await axios.delete(`http://localhost:4000/api/livestreamposts/${postId}`);
  ...
  return (
    <div className='row' style={{ padding: '10px', margin: '5px' }}>
      ...
      <div className='col-xl-8 col-lg-8 col-md-6 col-sm-12 pe-3  bg-info' style={{paddingTop: "50px"}}>
        <h2><strong>Youtube Stream Management Console</strong></h2>
      ...
      <div className='col-xl-4 col-lg-4 col-md-6 col-sm-12 pe-3 bg-info pt-8'>
        <CreateYTLiveStreamPost />
      </div>
    </div>
  );
};

export default HomeYTLiveStream;
